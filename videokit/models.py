...
class VideoField(models.FileField):
    ...
    def update_rotation_field(self, instance, force=False, *args, **kwargs):
        ...
        rotation_field_filled = not (
            self.rotation_field and not getattr(instance, self.rotation_field)
        )
    ...
    def update_mimetype_field(self, instance, force=False, *args, **kwargs):
        ...
        mimetype_field_filled = not (
            self.mimetype_field and not getattr(instance, self.mimetype_field)
        )
    ...
    def update_thumbnail_field(self, instance, force=False, *args, **kwargs):
        ...
        thumbnail_field_filled = not (
            self.thumbnail_field and not getattr(instance, self.thumbnail_field)
        )
    ...
    def __init__(
        ...
        **kwargs
        ...
        if not format in VideokitConfig.VIDEOKIT_SUPPORTED_FORMATS:
            raise ValueError(
                "Video format '%s' is not supported at this time by videokit." % format
            )

        super(VideoSpecField, self).__init__(
            verbose_name, name, blank=True, null=True, **kwargs
        )
